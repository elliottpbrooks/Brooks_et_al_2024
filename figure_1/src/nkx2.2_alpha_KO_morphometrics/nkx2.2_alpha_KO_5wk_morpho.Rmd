...
# Data Import and Tidying
  - Data from images were compiled from individual data spreadsheets onto the Masterlist data spreadshet below
```{r import}
data <- read_csv("/Volumes/Eliott_Brooks/Analysis/nkx_image_data/data/nkx22_gcgicre_5wk_mstrlst.csv", 
...
```{r}
#Create summary statitics table for expression area of all hormones measured and islet size
expression_summ <- data %>% 
  group_by(Genotype, Animal) %>% #grouped by genotype
  summarize(Islet_area = mean(islet_area, na.rm = TRUE), 
            sd_islet_expr = sd(islet_area, na.rm = TRUE)/sqrt(length(islet_area)),
            GCG = mean(gcg_area, na.rm = TRUE), 
            se_gcg_expr = sd(gcg_area, na.rm = TRUE)/sqrt(length(gcg_area)),
            INS = mean(ins_area, na.rm = TRUE), 
            se_ins_expr = sd(ins_area, na.rm = TRUE)/sqrt(length(ins_area)),
            SST = mean(sst_area, na.rm = TRUE), 
            se_sst_expr = sd(sst_area, na.rm = TRUE)/sqrt(length(sst_area)))
...
tiff(filename = paste0("~/Analysis/nkx_image_data/img/5wk_horm_expression.GCG.plot.STAT.tiff"), 
...
pdf(file = "~/Analysis/nkx_image_data/img/5wk_horm_expression.GCG.plot.STAT.pdf", 
     width = 7, height = 4)
horm_expression.GCG.plot
dev.off()
...
gcg.tom_summ <- data %>%
  ...
  summarise(gcg.tom_mean = mean(gcg.tom_sum, na.rm = TRUE), 
            gcg.tom_SE = sd(gcg.tom_sum, na.rm = TRUE)/sqrt(length(gcg.tom_sum)))


write_csv(gcg.tom_summ, "~/Analysis/nkx_image_data/data/gcg.tom_summ.csv")
...
tiff(filename = paste0("~/Analysis/nkx_image_data/img/5wk_gcg.tom_summ.plot.stats.tiff"), 
...
pdf(file = "~/Analysis/nkx_image_data/img/5wk_gcg.tom_summ.plot.stats.pdf", 
      width = 3, height = 4)
gcg.tom_summ.plot
dev.off()
...
bihorm_ins_gcg_prop <- bihorm_ins_gcg %>% 
  ...
  summarize(Percentage_bihorm_cells = mean(gcg_ins_bihorm),
            SE = sd(gcg_ins_bihorm, na.rm = TRUE)/sqrt(length(gcg_ins_bihorm))) %>%
...
bihorm_ins_gcg_BYislet_summ <- bihorm_ins_gcg_BYislet %>% 
  ...
  summarize(bihorm.per.islet = sum(all_gcg_ins)/islet_number,
            SE_bihorm.per.iselt =  sd(all_gcg_ins, na.rm = TRUE)/sqrt(length(all_gcg_ins))) %>%
  unique()



#Code not used in final analysis




#ins_gcg_bihorm_prop.plot <- ins_gcg_bihorm_plotting_df %>%
 # dplyr::select(Genotype, Percentage_bihorm_cells) %>%
 # ggplot(aes(x = Genotype, y = Percentage_bihorm_cells, fill = Genotype)) +
 # geom_bar(stat = "identity", position = position_dodge()) +
 # theme_cowplot() +
...
#Wilcoxen comparing the number of ins/gcg bihormonal cells in each islet

wilcox_ins.gcg.bihorm_mut.con <- wilcox.test(x = Mut_ins.gcg.bihorm$mean_all_gcg_ins, y = CON_ins.gcg.bihorm$mean_all_gcg_ins)

wilcox_ins.gcg.bihorm_mut.con_answer <- round(as.numeric(wilcox_ins.gcg.bihorm_mut.con[[3]]), digits = 4)
...
bihorm_sst_gcg_prop <- bihorm_sst_gcg %>% 
  ...
  summarize(Percentage_bihorm_cells = mean(gcg_sst_bihorm),            
            SE = sd(gcg_sst_bihorm, na.rm = TRUE)/sqrt(length(gcg_sst_bihorm))) %>%
...
bihorm_sst_gcg_BYislet_summ <- bihorm_sst_gcg_BYislet %>% 
  ...
  summarize(bihorm.per.islet = sum(all_gcg_sst)/islet_number,
            SE_bihorm.per.iselt =  sd(all_gcg_sst, na.rm = TRUE)/sqrt(length(all_gcg_sst))) %>%
  unique()
...
#Wilcoxen comparing the number of sst/gcg bihormonal cells in each islet

wilcox_sst.gcg.bihorm_mut.con <- wilcox.test(x = Mut_sst.gcg.bihorm$mean_all_gcg_sst, y = CON_sst.gcg.bihorm$mean_all_gcg_sst)

wilcox_sst.gcg.bihorm_mut.con_answer <- round(as.numeric(wilcox_sst.gcg.bihorm_mut.con[[3]]), digits = 3)
...
#export plot
tiff(filename = paste0("~/Analysis/nkx_image_data/img/5wk_INS_bihorm_byIslet_plotting.plot.STAT.tiff"), 
     units = "in", width = 3, height = 4, 
     res = 300, compression = "lzw")
INS.bihomr_byIslet_plotting.plot_stripe
dev.off()

pdf(file = "~/Analysis/nkx_image_data/img/5wk_INS_bihorm_byIslet_plotting.plot.STAT.pdf", 
    width = 3, height = 4)
INS.bihomr_byIslet_plotting.plot_stripe
dev.off()
```
...
#export plot
tiff(filename = paste0("~/Analysis/nkx_image_data/img/5wk_SST_Bihorm_byIslet_plotting.plot.STAT_stripe.tiff"), 
     units = "in", width = 3, height = 4, 
     res = 300, compression = "lzw")
SST.bihomr_byIslet_plotting.plot_stripe
dev.off()

pdf(file = "~/Analysis/nkx_image_data/img/5wk_SST_Bihorm_byIslet_plotting.plot.STAT_stripe.pdf", 
     width = 3, height = 4)
SST.bihomr_byIslet_plotting.plot_stripe
dev.off()
